...
def update_member_mc_time(time, chat_id):
    conn = psycopg2.connect(
        dbname="bauman_festival_bot",
        user="admin",
        password="admin",
    ...
    try:
        cursor.execute(
            "UPDATE member SET mc_time = %s WHERE tg = %s;",
            (chat_id, time)
        )
        conn.commit()
    except Exception as e:
...
def make_kb_mc_times():
    kb = [
        [
            types.KeyboardButton(text=b) for b in times
        ],
    ]
...
class UserStatesMcTimes(StatesGroup):
    McStart = State()
    McConf = State()
...
async def user_fest_rega_start(message: Message, state: FSMContext):
    if message.text == buttons[0]:
        ...
        await state.set_state(UserStatesMcTimes.McStart)
        kb = make_kb_mc_times()
        await message.answer(
            text="выберете время:",
            reply_markup=kb
...
@router.message(UserStatesMcTimes.McStart, F.text.in_(times))
async def user_fest_mc_rega_start(message: Message, state: FSMContext):
    if message.text != times[3]:
        await message.answer(text=f"ждем вас на МК творчества в {message.text}!")
        update_member_mc_time(message.text, message.chat.id)
        await state.set_state(UserStates.Start)
    else:
        await message.answer(text=f"жаль, приходите, если передумаете!{message.text}!")
